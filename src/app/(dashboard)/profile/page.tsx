...
function InfoItem({ icon, label, value, hint }: {
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-(--panel-border-subtle) bg-(--panel-inset)/55 p-3.5">
...
export default async function ProfilePage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1680px] space-y-5 p-4 text-(--panel-text) sm:p-5 lg:p-6">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-(--panel-border) bg-(--panel) p-4 shadow-sm sm:p-5">
        <div className="absolute inset-x-0 top-0 h-1 bg-indigo-500" />
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-indigo-600 text-xl font-bold text-white">{initials}</div>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        <section className="rounded-2xl border border-(--panel-border) bg-(--panel) p-4 shadow-sm sm:p-5">
          <div className="mb-4"><h2 className="text-base font-semibold">ព័ត៌មានគណនី</h2><p className="mt-1 text-xs text-(--panel-text-subtle)">ព័ត៌មានដែលប្រើសម្រាប់ការទំនាក់ទំនង និងកំណត់សិទ្ធិ</p></div>
        ...
        <section className="rounded-2xl border border-(--panel-border) bg-(--panel) p-4 shadow-sm sm:p-5">
          <div className="mb-4 flex items-start gap-3"><div className="rounded-xl bg-violet-500/10 p-2.5 text-violet-600 dark:text-violet-300"><IdCard className="h-5 w-5" /></div><div><h2 className="text-base font-semibold">អត្តសញ្ញាណប័ណ្ណ</h2><p className="mt-1 text-xs text-(--panel-text-subtle)">ឯកសារផ្ទៀងផ្ទាត់អត្តសញ្ញាណ</p></div></div>
          {!hasIdImages ? (
